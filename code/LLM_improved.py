...
try:
    faiss_module_path = os.path.abspath('D:/同步文件/课程作业/2025秋/人工智能/HW2/.aux/数据库-哈利波特/harrypotter_agent_FAISS.py')
...
while True:
    ...
    try:
        ...
        tool_call = completion.choices[0].message.tool_calls[0] if completion.choices[0].message.tool_calls else None
        ...
        if tool_call and faiss_module:
            try:
                ...
                # 执行工具调用 - 使用正确的函数名 question_to_context
                tool_response = faiss_module.question_to_context(**tool_args)
                
                # 将工具返回的信息添加到消息历史中 - 使用可序列化的格式
                tool_call_dict = {
                    "id": tool_call.id,
                    "type": "function",
                    "function": {
                ...
                # 直接打印最终回复
                print("\033[93mAgent:\033[0m")
                print(response)
                
            except Exception as e:
                print(f"\033[93m警告：工具调用失败: {e}\033[0m")
        
        # 添加助手回复到消息历史 - 确保符合OpenAI标准接口
        if tool_call:
            # 如果有工具调用，使用正确的格式
        ...
        if harry_potter_used and 'all_extracted_contexts' in locals() and all_extracted_contexts and 'sufficient_context_found' in locals() and sufficient_context_found:
            try:
                # 添加用户问题，要求agent解释如何利用工具信息
                explanation_prompt = {
                    "role": "user",
                    "content": "请解释你刚才的回答是如何利用哈利波特搜索工具提供的信息的？具体说明你参考了哪些原文片段，以及这些信息如何帮助你回答问题。"
                }
                messages.append(explanation_prompt)
                
                # 显示用户问题
                print("\033[92mUser:\033[0m")
                print("请解释你刚才的回答是如何利用哈利波特搜索工具提供的信息的？具体说明你参考了哪些原文片段，以及这些信息如何帮助你回答问题。")
                print()
        ...
        # 在参考资料分析完成后，进行消息清理
        # 将工具返回的内容替换为"NONE"以节省存储空间
        for i, msg in enumerate(messages):
            if msg.get('role') == 'tool':
                # 保留工具消息结构，但将内容替换为"NONE"
                messages[i] = {
                    'role': 'tool',
                    'content': '已清除内存',
